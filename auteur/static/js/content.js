document.addEventListener("DOMContentLoaded", (event) => {
    ...
    $("#checkpointform").submit(function (event) {
        ...
        fetch(SCRIPT_ROOT + '/create_checkpoint', {
            method: "POST",
            headers: {
                "X-CSRFToken": $('meta[name=csrf-token]').attr('content')
            },
            ...
            .then(response => {
                if (!response.ok) {
                    console.log('Problem with create_checkpoint: %o', response.json());
                }
                return response.json();
            })
            .then(data => {
                if (data.status) {
                    ...
                    formEl.reset();
                    $('#statusbar').html(data.status_text);
                    $(document).trigger('auteur:saved');
                } else {
                    const errors = data.label_errors || [];
                    $('#checkpointFormErrors').removeClass('d-none').html(errors.join('<br>'));
                }
            })
            .catch(error => console.error('There was an error with the Create Checkpoint Fetch operation: ', error));
    });

    // Clear any stale validation message each time the modal is reopened.
    checkpointModalEl.addEventListener('show.bs.modal', function () {
        $('#checkpointFormErrors').addClass('d-none').empty();
    });
