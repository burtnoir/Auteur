	...
	function() {
	    ...
	    $('#tree').on(
			...
			$.getJSON(SCRIPT_ROOT + '/get_section', {
			    ...
			    CKEDITOR.instances.sectiontext
				    .setData(data.section_text);
		...
		$.ajax({
			...
			if (r.status) {
			    ...
			    $('#descriptiongroup, #namegroup').removeClass(
				    'has-error');
